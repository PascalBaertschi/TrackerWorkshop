...
#include "ColorParser.h" // this one may already exist in CMSSW
#include "Trend.h"
...
Run2Lumi::Run2Lumi (fs::path file, int first, int last, float scale) :
    firstRun(first), lastRun(last), scaling(scale)
{
    cout << __func__ << endl;
    assert(first < last);
    ...
    JSON(json), GetLumi(GetLumiFunctor)
{
    ...
    const char * axistitles = Form(";Delivered integrated luminosity  [fb^{-1} ];%s", ytitle);
...
Trend::~Trend ()
{
    ...
    for (auto& type: JSON) {
        ...
        while (runIt != runs->end() && labelIt != labels->end()) {
            ...
            auto posX = l+(lumi/totLumi)/(l+1+r)+0.01;
...
TString Trend::CMS = "#scale[1.2]{#bf{CMS}} #it{Internal}", 
        Trend::lumi = "pp collisions (13.6 TeV)";
float Trend::fontsize = 0.05;
