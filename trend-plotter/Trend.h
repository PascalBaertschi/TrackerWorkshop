...
struct Run2Lumi {
    ...
    const float scaling; //!< e.g. conversion from /pb to /fb
...
public:
    ...
    Run2Lumi
        (fs::path file, //!< path to a 2-column file with 6-digit run number and lumi
         int first, //!< 6-digit run number (included)
         int last, //!< 6-digit run number (excluded)
         float scaling = 1 //!< unit conversion if necessary (otherwise assuming /fb)
         );
